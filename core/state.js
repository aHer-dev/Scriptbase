...
const undoStack = [];  // JSON-Snapshots vor jedem update()
const redoStack = [];  // JSON-Snapshots nach undo()
...
export function update(mutator) {
  ...
  undoStack.push(JSON.stringify({ course, activeChapterId }));
...
export function undo() {
  ...
  redoStack.push(JSON.stringify({ course, activeChapterId }));
...
export function setMode(newMode) {
  mode = newMode;
  try { localStorage.setItem('mode', newMode); } catch (e) { /* ignore */ }
  modeListeners.forEach(fn => { try { fn(mode); } catch (e) { console.error('Mode-Subscriber-Fehler:', e); } });
}
