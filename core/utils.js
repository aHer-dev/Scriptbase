...
/**
 * ISO-Timestamp für createdAt/updatedAt-Felder.
 * @returns {string}
 */
export function now() {
...
/**
 * Konvertiert versehentlich als Text gespeicherte HTML-Inline-Tags zurück zu echtem HTML.
 * Betrifft z.B. &lt;strong&gt; → <strong>, das passiert wenn Inhalt als Plain-Text
 * statt als formatierter Text in den Editor eingefügt wurde.
 * Nur sichere Inline-Tags werden konvertiert; andere &lt;...&gt; Muster bleiben.
 *
 * @param {string} html
 * @returns {string}
 */
export function normalizeContent(html) {
  ...
  return html.replace(
    /&lt;(\/?(strong|em|b|i|u|s|del|ins|mark|code|sub|sup|br\s*\/?)(\s[^<>]*)?)&gt;/gi,
    "<$1>"
  );
}
